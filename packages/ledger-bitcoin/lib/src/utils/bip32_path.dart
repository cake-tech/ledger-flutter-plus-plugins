...
class BIPPath {
  ...
  BIPPath.fromPathArray(this._path) {
    if (_path.isEmpty) throw Exception('Path must contain at least one level');
  }
  ...
  factory BIPPath.fromString(String text, {bool requireRoot = false}) {
    ...
    for (var i = 0; i < path.length; i++) {
      ...
      if (ret[i] >= hardened) throw Exception('Invalid child index');
  ...
  List<int> get hardenedPath {
    for (var i = _path.length - 1; i >= 0; i--) {
      if (_path[i] >= 0x80000000) return _path.sublist(0, i + 1);
    }
    return [];
  }

  BIPPath toHardenedBIPPath() => BIPPath.fromPathArray(hardenedPath);
